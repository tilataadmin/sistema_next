...
**Reemplaza:** v0.7 … v0.11 (todos retirados)
**Estado:** Fases 1–3 completas en DEV. Fase 4 parcial (`form.html` listo; `leads-review.html` en espera). Fase 5 (Experiencias) completa en DEV. **Fase 6 completa en DEV** (catálogo de documentos, satélite de documentos y **Paso 4 completo** `step4-form.html`). Esquema del Paso 4 replicado en PROD. Ningún HTML del módulo desplegado a PROD todavía.

---

## 0. QUÉ CAMBIA RESPECTO A v0.11
...
`Aspirantes` · `Configuración general del módulo` · `Plantillas de correo` · `Experiencias` · `Fuentes de contacto` · `Ferias de preescolares` · `Nacimiento - grados` · `Gestionar acciones de jardines` · `Jardines infantiles` · `Razones de pérdida` · `Tipos de referidos` · `Subir archivos de campañas` · **`Catálogo de documentos`** (activado en Fase 6)
...
| Financiero del aspirante | `applicant-financial.html` (satélite → sin ruta) | Fase 7 |
...
| **7** | `applicant-financial.html` (Paso 5 + acuerdos) | Pendiente (siguiente fase disponible) |
...
**Estado de PROD (v0.12):** el esquema completo, el trigger, los datos sembrados (11 pasos, 6 resultados, config, 11 plantillas), el bucket, **y el esquema del Paso 4** (contenedor `forms`, 50 `form_fields` con condicionales, `step4_form_id` fijado) **ya están en PROD**. **Ningún HTML del módulo está desplegado a PROD todavía** — siguen solo en `developmen`.

**Despliegue del módulo a PROD (pendiente, sesión aparte):** requiere subir todos los HTML del módulo (no solo `step4-form.html`; también `form.html`, la ficha, catálogos, satélites, `module-config.html`, etc.), sembrar el catálogo de documentos completo del Excel y los años habilitados, activar los permisos en PROD, y verificar los endpoints. Es un despliegue grande, a planear como sesión propia.
